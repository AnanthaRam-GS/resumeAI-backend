...
describe('AI services', () => {
	...
	it('parses Groq JSON responses', async () => {
		...
		await expect(requestGroqJson<{ hello: string }>({ userPrompt: 'return json' })).resolves.toEqual({
			hello: 'world',
		});
	});
	...
	it('parses Gemini JSON responses', async () => {
		...
		await expect(requestGeminiJson<{ hello: string }>({ userPrompt: 'return json' })).resolves.toEqual({
			hello: 'gemini',
		});
	});
