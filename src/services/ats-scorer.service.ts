...
export const getTaxonomySkills = (): SkillTaxonomySkill[] => {
  return flattenTaxonomySkills();
};

export const getTaxonomyKeywords = (): string[] => {
  const keywords = flattenTaxonomySkills().flatMap((skill) => [
    skill.name,
    ...(skill.synonyms ?? []),
  ]);
...
export const findMatchingKeywords = (text: string, keywords: string[]): string[] => {
  return unique(
    keywords
      .filter((keyword) => textIncludesKeyword(text, keyword))
...
export const scoreAtsMatch = ({
  jobDescription,
  resumeText,
  extractedEntities,
}: AtsScoreInput): ATSResult => {
  ...
  const resumeKeywords = findMatchingKeywords(resumeText, [
    ...getTaxonomyKeywords(),
    ...jobKeywords,
  ]);

  const foundRequired = requiredSkills.filter((keyword) =>
    textIncludesKeyword(resumeText, keyword),
  );
  const foundPreferred = preferredSkills.filter((keyword) =>
    textIncludesKeyword(resumeText, keyword),
  );
  ...
  const maxScore = Math.max(
    requiredSkills.length * 4 + preferredSkills.length * 2 + techStack.length,
    1,
  );
